...
public class CameraPreviewView extends FrameLayout {
    ...
    private static final long OPEN_CAMERA_DELAY = 350;    //开启相机延迟
    ...
    private ProgressBar mProgressBar;   //添加一个进度条
    ...
    private void initTextView(Context context) {
        ...
        rl.setId(R.id.prompt_parent_ll);
        //设置为半透明
        rl.setBackgroundColor(getResources().getColor(R.color.transparent));
        ...
        //
        mProgressBar.setId(R.id.my_progress_view);
        /**
         * 先用系统的，后面定制
         */
        Drawable progressBarDrawable = getContext().getResources().getDrawable(R.drawable.progressbar_horizontal_2);
        // 定义
        ...
        RelativeLayout.LayoutParams relLayoutParams = new RelativeLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, 3);
        relLayoutParams.addRule(RelativeLayout.ALIGN_PARENT_BOTTOM);
        relLayoutParams.addRule(RelativeLayout.CENTER_HORIZONTAL, RelativeLayout.TRUE);
        //添加元素进rela
        rl.addView(mProgressBar, relLayoutParams);

        textViewPrompt = new TextView(context);
        textViewPrompt.setId(R.id.text_view_prompt);
        textViewPrompt.setTextSize(14);
        textViewPrompt.setPadding(15, 5, 15, 5);
        textViewPrompt.setBackgroundColor(Color.parseColor("#4d000000"));
        textViewPrompt.setTextColor(Color.WHITE);//getResources().getColor(R.color.greenyellow)
        textViewPrompt.setText(R.string.text_view_prompt);
        RelativeLayout.LayoutParams relLayoutParams2 = new RelativeLayout.LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT, ViewGroup.LayoutParams.WRAP_CONTENT);
        relLayoutParams2.addRule(RelativeLayout.ALIGN_BOTTOM, R.id.my_progress_view);
//        relLayoutParams2.addRule(RelativeLayout.ABOVE, R.id.my_progress_view);
        //relLayoutParams2.addRule(RelativeLayout.CENTER_VERTICAL);
        relLayoutParams2.addRule(RelativeLayout.CENTER_HORIZONTAL);
        relLayoutParams2.bottomMargin = 50;
        rl.addView(textViewPrompt, relLayoutParams2);

        addView(rl, new LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT, ViewGroup.LayoutParams.WRAP_CONTENT, Gravity.BOTTOM));

        hideProgressBar(true);    //先隐藏
        hideText();
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        int height = (int) (width / viewWHRatio);               // 设置高度
//        int height = ((WindowManager) getContext()
//                .getSystemService(Context.WINDOW_SERVICE)).getDefaultDisplay().getHeight() / 2;
...
//            }
    ...
    /**
     * 进度条增加
     * 进度条应该加多一倍的数字。避免走了一半，往回走。
     */
    public void progressBarPlus(int timeLong, int timeCount) {
        int timeCount2 = timeLong - timeCount;
        if (mProgressBar != null) {
            mProgressBar.setProgress(timeCount);
            Log.d("progress时间：第一个进度条", timeCount + "");
            mProgressBar.setSecondaryProgress(timeCount2);
            Log.d("progress时间：第二个进度条", timeCount2 + "");
            //需要新建一个子线程保证能更新主线程的textview
        } else {
            // textViewPrompt.setVisibility(View.GONE);
            Log.d("progress", "progress = null");
        }
    }
     ...
     * @param tx
     */
    public void changText(final String tx, final int color) {
        textViewPrompt.post(new Runnable() {
            @Override
            public void run() {
                if (tx.equals("松开取消")) {
                    textViewPrompt.setBackgroundColor(Color.parseColor("#CCFF6094"));
                } else {
                    textViewPrompt.setBackgroundColor(Color.parseColor("#4d000000"));
                }
                textViewPrompt.setText(tx);
                textViewPrompt.setTextColor(color);
            }
        });
    }
     ...
     * 进度条减少
     */
    public void progressBarReduce(int timeCount) {
        if (mProgressBar != null) {
            mProgressBar.setProgress(timeCount);
        } else {
            mProgressBar.setProgress(0);
