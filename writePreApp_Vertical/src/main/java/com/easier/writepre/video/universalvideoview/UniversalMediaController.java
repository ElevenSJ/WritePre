...
package com.easier.writepre.video.universalvideoview;
...
//    private boolean mFullscreenEnabled = false;
    ...
    private boolean setProgressEnable = true;
    ...
    public UniversalMediaController(Context context, AttributeSet attrs) {
        ...
        mScalable = a.getBoolean(R.styleable.UniversalMediaController_uvv_scalable, true);
        a.recycle();
        init(context);
    }
    ...
    private void initControllerView(View v) {
        ...
        mLeftTitle = (TextView) v.findViewById(R.id.left_title);
        if (mLeftTitle != null) {//左侧按钮仅在半屏状态下可见
            mLeftTitle.setOnClickListener(mRightTitleListener);
        }
    ...
    //仅半屏时才显示
    private OnClickListener mRightTitleListener = new OnClickListener() {
        public void onClick(View v) {
            if (!mIsFullScreen) {
                if (mPlayer != null)
                    mPlayer.finishVideoView();
            }
...
//            mCenterPlayButton.setVisibility(VISIBLE);
    ...
    void updateBackButton() {
        mBackButton.setVisibility(mIsFullScreen ? View.VISIBLE : View.GONE);
        if (showLeftView) {
            mLeftTitle.setVisibility(mIsFullScreen ? View.INVISIBLE : View.VISIBLE);
        } else {
