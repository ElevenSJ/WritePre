...
public class HanziToPinyin {
        ...
        synchronized (HanziToPinyin.class) {
            ...
            for (int i = 0; i < locale.length; i++) {
                if (locale[i].equals(Locale.CHINESE)) {
                    ...
                    if (DEBUG) {
                        LogUtils.d("Self validation. Result: " + doSelfValidation());
                    }
                    sInstance = new HanziToPinyin(true);
                    return sInstance;
                }
            }
            LogUtils.w("There is no Chinese collator, HanziToPinyin is disabled");
    ...
    private static boolean doSelfValidation() {
        ...
        for (char c : UNIHANS) {
            ...
            if (cmp >= 0) {
                LogUtils.e("Internal error in Unihan table. " + "The last string \"" + lastString
