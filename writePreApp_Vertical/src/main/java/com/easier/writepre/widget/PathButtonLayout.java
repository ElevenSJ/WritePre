...
public class PathButtonLayout extends RelativeLayout {
	...
	private AnimationsUtil myani; // 動畫類
	 ...
	 */
	public PathButtonLayout(Context context, AttributeSet attrs, int defStyle) {
	 ...
	 *            動畫耗時
	 */
	public void init(int[] imgResId, final int showhideButtonId, byte pCode,
			int radius, final int durationMillis) {
		...
		RelativeLayout.LayoutParams buttonlps = new RelativeLayout.LayoutParams(
				DisplayUtil.dip2px(mycontext, 35),
				DisplayUtil.dip2px(mycontext, 35));
		...
		myani = new AnimationsUtil(rl1, pCode, radius);
		rlButton.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View v) {
				if (areButtonsShowing) {
					myani.startAnimationsOut(duretime);
					cross.startAnimation(AnimationsUtil.getRotateAnimation(
							-270, 0, duretime));
					rlButton.setBackgroundResource(showhideButtonId);
				} else {
					myani.startAnimationsIn(duretime);
					cross.startAnimation(AnimationsUtil.getRotateAnimation(0,
							-270, duretime));
					rlButton.setBackgroundResource(R.drawable.sign_rotate);
				}
				areButtonsShowing = !areButtonsShowing;
			}
		});

		cross.startAnimation(AnimationsUtil.getRotateAnimation(0, 360, 200));
		this.addView(rl1);
	...
	public void collapse() {
		myani.startAnimationsOut(duretime);
		cross.startAnimation(AnimationsUtil.getRotateAnimation(-270, 0,
				duretime));
		areButtonsShowing = false;
		rlButton.setBackgroundResource(R.drawable.sign_off);
	}
	...
	public void expand() {
		myani.startAnimationsIn(duretime);
		cross.startAnimation(AnimationsUtil.getRotateAnimation(0, -270,
				duretime));
