/*
 * Android Wheel Control. https://code.google.com/p/android-wheel/ Copyright
 * 2010 Yuri Kanivets Licensed under the Apache License, Version 2.0 (the
...
import com.easier.writepre.adapter.WheelAdapter;
 ...
 * 
 * @author chenhong
 * @version [DrivingTraining, 2014-9-9]
 */
public class MyWheelView extends View {
	...
	public void setCurrentItem(int index, boolean animated) {
		...
		if (index != Integer.parseInt(currentItem)) {
			if (animated) {
				scroll(index - Integer.parseInt(currentItem),
						SCROLLING_DURATION);
			} else {
				invalidateLayouts();

				int old = Integer.parseInt(currentItem);
				currentItem = index + "";

				notifyChangingListeners(old, Integer.parseInt(currentItem));
	...
	private String buildText(boolean useCurrentValue) {
		...
		for (int i = Integer.parseInt(currentItem) - addItems; i <= Integer
				.parseInt(currentItem) + addItems; i++) {
			if (useCurrentValue || i != Integer.parseInt(currentItem)) {
				String text = getTextItem(i);
				if (text != null) {
					itemsText.append(text);
				}
			}
			if (i < Integer.parseInt(currentItem) + addItems) {
				itemsText.append("\n");
			}
		}
	...
	private int getMaxTextLength() {
		...
		for (int i = Math.max(Integer.parseInt(currentItem) - addItems, 0); i < Math
				.min(Integer.parseInt(currentItem) + visibleItems,
	...
	private void createLayouts(int widthItems, int widthLabel) {
		...
		if (!isScrollingPerformed
			...
			String text = getAdapter() != null ? getAdapter().getItem(
					Integer.parseInt(currentItem)) : null;
	...
	private void doScroll(int delta) {
		...
		int pos = Integer.parseInt(currentItem) - count;
		if (isCyclic && adapter.getItemsCount() > 0) {
			...
			//
			if (pos < 0) {
				count = Integer.parseInt(currentItem);
				pos = 0;
			} else if (pos >= adapter.getItemsCount()) {
				count = Integer.parseInt(currentItem) - adapter.getItemsCount()
						+ 1;
		...
		if (pos != Integer.parseInt(currentItem)) {
	...
	private final SimpleOnGestureListener gestureListener = new SimpleOnGestureListener() {
		...
		public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX,
				float velocityY) {
			lastScrollY = Integer.parseInt(currentItem) * getItemHeight()
					+ scrollingOffset;
	...
	private void justify() {
		...
		boolean needToIncrease = offset > 0 ? Integer.parseInt(currentItem) < adapter
				.getItemsCount() : Integer.parseInt(currentItem) > 0;
