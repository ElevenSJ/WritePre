...
public class DragImageView extends ImageView {
	...
	void setScale(float scale) {
		...
		if (scale > 1 && this.getWidth() <= MAX_W) {
			...
			if (current_Top <= 0 && current_Bottom >= screen_H) {
				// LogUtils.e("jj", "��Ļ�߶�=" + this.getHeight());
				isControl_V = true;// ������ֱ���
			} else {
				isControl_V = false;
			}
	...
	class MyAsyncTask extends AsyncTask<Void, Integer, Void> {
		...
		private final float scale_WH;// ��ߵı���
		...
		private final float STEP = 8f;// ����

		private final float step_H, step_V;// ˮƽ��������ֱ����
		...
		protected Void doInBackground(Void... params) {
			...
			while (current_Width <= screen_W) {
				...
				LogUtils.e("top=" + top + ",bottom=" + bottom + ",left="
						+ left + ",right=" + right);
