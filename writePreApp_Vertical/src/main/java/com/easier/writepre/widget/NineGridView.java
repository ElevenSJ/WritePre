...
public class NineGridView<T> extends ViewGroup {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (mImageInfo != null && mImageInfo.size() > 0) {
            if (mImageInfo.size() == 1) {
                ...
                gridHeight = (int) (gridWidth / singleImageRatio*0.7f);
            } else if (mImageInfo.size() == 2) {
                gridWidth = gridHeight = (totalWidth - gridSpacing * 2) / 8 * 3;
            } else {
     ...
     * 设置适配器
     */
    public void setAdapter(@NonNull NineGridViewAdapter adapter, boolean isVideo) {
    ...
    public void setAdapter(@NonNull NineGridViewAdapter adapter) {
        mAdapter = adapter;
        List<T> imageInfo = adapter.getImageInfo();
        ...
        if (mImageInfo == null) {
            for (int i = 0; i < imageCount; i++) {
                NineGridViewWrapper iv = getImageView(i);
                if (iv == null) return;
     ...
     */
    public NineGridViewWrapper getImageView(final int position) {
    ...
    public interface ImageLoader<T> {
        void onDisplayImage(Context context, NineGridViewWrapper imageView, T url);
