...
public class AudioFocusChangeManager {//
    ...
    private static Handler handler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            if (flag)
                mode = 2;
            updateMode(WritePreApp.getApp().getApplication(), mode);
        }

    };

    static {
        audioManger = (AudioManager) WritePreApp.getApp().getApplication().getSystemService(
                Context.AUDIO_SERVICE);
    }

    public static void checkVolume() {//
        if (audioManger.getStreamVolume(AudioManager.STREAM_MUSIC) == 0) {
            Toast.makeText(WritePreApp.getApp().getApplication(), R.string.volumeTips,Toast.LENGTH_SHORT).show();
        }
    }
    ...
    public static void updateToNormalMode() {//
        updateMode(WritePreApp.getApp().getApplication(), AudioManager.MODE_NORMAL);
    }
    ...
    public static boolean registerListener() {
        if (sensorListener == null)
            sensorListener = new ProximitySensorListener(WritePreApp.getApp().getApplication(),
                    handler);
        if (sensorListener == null) {
            return false;
        } else {
